...
use crate::{enums::{TeamID, PlayerType}, comms::{PlayerData, RadarData, ArcRwlockRadarData, BombData}, structs::Vec3};
...
pub async fn run(radar_data: ArcRwlockRadarData, connector: Connector, pcileech_device: String) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if ingame {
            ...
            let (bomb_can_defuse, bomb_defuse_end) = {
                if data.bomb_planted && !data.bomb_exploded && !data.bomb_defused {
                    if let (Some(bomb_stamp), Some(defuse_stamp)) = (data.bomb_planted_stamp, data.bomb_defuse_stamp) {
                        ...
                        (time_left - defuse_left > 0.0, time_left - defuse_left)
                    } else {
                        (false, 0.0)
                    }
                } else {
            ...
            let mut bomb_data = BombData {
                planted: data.bomb_planted,
                dropped: data.bomb_dropped,
                pos: Vec3::default(),
                defused: data.bomb_defused,
                exploded: data.bomb_exploded,
                being_defused: data.bomb_being_defused,
                can_defuse: bomb_can_defuse,
                defuse_length: data.bomb_defuse_length,
                timeleft: bomb_explode_timeleft,
                defuse_end: bomb_defuse_end,
            };
            ...
            if local_data.health > 0 {
                ...
                entity_data.push(
                    PlayerData {
                        ...
                        has_bomb,
                        has_awp: local_data.has_awp,
                        is_scoped: local_data.is_scoped,
                    }
                );
            }
            ...
            for (controller, pawn) in &data.players {
                ...
                entity_data.push(
                    PlayerData {
                        ...
                        player_type,
                        has_bomb,
                        has_awp: player_data.has_awp,
                        is_scoped: player_data.is_scoped,
                    }
                );
            }
